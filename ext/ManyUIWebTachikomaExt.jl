# ManyUIWebTachikomaExt -- the Tachikoma frontend.
#
# This is the second frontend for ManyUIWeb's neutral transport, and the
...
# bundle and the wire protocol are all ManyUIWeb's; only the per-connection
...
module ManyUIWebTachikomaExt

using ManyUIWeb
import ManyUIWeb: AbstractSession, AbstractFrontend, ServerConfig,
...
function session_control!(s::TachikomaSession, m::ManyUIWeb.ControlMessage)
    if m.kind === ManyUIWeb.ControlKind.RESIZE && m.width > 0 && m.height > 0
...
function serve_tachikoma(factory; host = ManyUIWeb.Sockets.localhost,
                         port::Int = 8000, title::AbstractString = "Tachikoma",
                         default_size = ManyUI.Size(80, 24), kwargs...)
