# backend_tests.jl -- WebBackend, the out-of-tree half of DualUI's launch
# seam.
#
...
@testitem "backend: WebBackend is a DualUI.Backend" begin
    using DualUIWeb
    using DualUI

    @test WebBackend <: DualUI.Backend
    @test WebBackend() isa DualUI.Backend
...
@testitem "backend: WebBackend defines no make_driver" begin
    ...
    @test_throws MethodError DualUI.make_driver(WebBackend())
end
...
@testitem "backend: launch(wait = false) returns a live WebServer" tags=[:socket] begin
    ...
    try
        ...
        @test isopen(server)
        @test DualUIWeb.bound_port(server) > 0
    finally
        close(server)
    end
    @test !isopen(server)
...
@testitem "backend: launch threads AppConfig into every session" tags=[:socket] begin
    ...
    cfg = DualUI.AppConfig(; title = "threaded", min_size = DualUI.Size(9, 2),
    ...
    server = launch(() -> Container(Label("x"));
                    ...
                    wait = false)
    try
        s = DualUIWeb.create_session!(server)
        @test s !== nothing
        @test s.app.config.title == "threaded"
        @test s.app.config.min_size == DualUI.Size(9, 2)
...
@testitem "backend: serve's own config still reaches sessions" tags=[:socket] begin
    ...
    server = serve(() -> Container(Label("x"));
                   port = 0, title = "legacy",
                   min_size = DualUI.Size(7, 3))
    try
        s = DualUIWeb.create_session!(server)
        @test s.app.config.title == "legacy"
        @test s.app.config.min_size == DualUI.Size(7, 3)
    finally
        DualUI.stop!(server)
    end
end
...
@testitem "backend: an explicit app config wins over title/min_size" begin
    ...
    cfg = ServerConfig(; title = "ignored", min_size = DualUI.Size(2, 2),
                         app = DualUI.AppConfig(; title = "wins",
                                                  min_size = DualUI.Size(5, 5)))
    @test cfg.app.title == "wins"
    @test cfg.app.min_size == DualUI.Size(5, 5)
...
@testitem "backend: launch calls the factory once per session, not once" tags=[:socket] begin
    ...
    try
        ...
        DualUIWeb.create_session!(server)
        @test calls[] == 1
        DualUIWeb.create_session!(server)
