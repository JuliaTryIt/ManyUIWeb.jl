...
@testitem "backend: WebBackend is a ManyUITUI.Backend" begin
    using ManyUIWeb, ManyUITUI
    using ManyUI, ManyUITUI

    @test WebBackend <: ManyUITUI.Backend
    @test WebBackend() isa ManyUITUI.Backend
...
@testitem "backend: WebBackend defines no make_driver" begin
    ...
    @test_throws MethodError ManyUITUI.make_driver(WebBackend())
end
...
@testitem "backend: launch threads AppConfig into every session" tags=[:socket] begin
    ...
    cfg = ManyUITUI.AppConfig(; title = "threaded", min_size = ManyUITUI.Size(9, 2),
    ...
    try
        ...
        @test s.app.config.min_size == ManyUITUI.Size(9, 2)
...
@testitem "backend: serve's own config still reaches sessions" tags=[:socket] begin
    ...
    server = serve(() -> Container(Label("x"));
                   port = 0, title = "legacy",
                   min_size = ManyUITUI.Size(7, 3))
    try
        s = ManyUIWeb.create_session!(server)
        @test s.app.config.title == "legacy"
        @test s.app.config.min_size == ManyUITUI.Size(7, 3)
    finally
...
@testitem "backend: an explicit app config wins over title/min_size" begin
    ...
    cfg = ServerConfig(; title = "ignored", min_size = ManyUITUI.Size(2, 2),
                         app = ManyUITUI.AppConfig(; title = "wins",
                                                  min_size = ManyUITUI.Size(5, 5)))
    @test cfg.app.title == "wins"
    @test cfg.app.min_size == ManyUITUI.Size(5, 5)
